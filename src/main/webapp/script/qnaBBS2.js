...
function read(p1, p2) {
	...
	let param = "/Proj_OnedayClass/qnaBBS/QnaRead2.jsp?qNum="+p1;
	     param += "&nowPage="+p2;
	     param += "&keyField="+p3;
	     param += "&keyWord="+p4 ; 
...
function movePage(p1) {    // 페이지 이동
	...
	let param = "/Proj_OnedayClass/qnaBBS/QnaList2.jsp?nowPage="+p1;	    
	     param += "&keyField="+p3;
	     param += "&keyWord="+p4 ; 
...
function moveBlock(p1, p2) {    // 블럭 이동
	...
	let param = "/Proj_OnedayClass/qnaBBS/QnaList2.jsp?nowPage="+(pagePerBlock*(pageNum-1)+1);
	     param += "&keyField="+p3;
	     param += "&keyWord="+p4 ; 
...
$(function(){
	...
	$("#postBtn").click(function(){
		// ajax 통신
		$.ajax({
		    url : "/Proj_OnedayClass/qnaBBS/QnaPost2.jsp",      // 컨트롤러에서 대기중인 URL 주소이다.
		    type : "GET",            // HTTP method type(GET, POST) 형식이다.
		...
		$("#regBtn").click(function(event){
			...
			if (qTitle == "") {
			    ...
			    $.ajax({             
			        ...
			        url: "/Proj_OnedayClass/qnaBBS/QnaPostProc2.jsp",        
			        data: formData,          
			        processData: false,    
			        contentType: false,      
			        cache: false,           
			        success: function () { 
			        	$.ajax({
						    url : "/Proj_OnedayClass/qnaBBS/QnaList2.jsp",      // 컨트롤러에서 대기중인 URL 주소이다.
						    type : "GET",            // HTTP method type(GET, POST) 형식이다.
	...
	$("button#delBtn").click(function(){
		...
		let param = "/Proj_OnedayClass/qnaBBS/QnaDelete2.jsp?";
		...
		$.ajax({
		    url : param,      // 컨트롤러에서 대기중인 URL 주소이다.
		    type : "GET",            // HTTP method type(GET, POST) 형식이다.
		}).done(function(qnaDelete){
			$("#tblArea").html(qnaDelete);
		});
	});
	...
	$("td.read>button#modBtn").click(function(){
		...
		let param = "/Proj_OnedayClass/qnaBBS/QnaUpdate2.jsp?";
			param += "qNum="+qNum;
	...
	$("td.read>button#replyBtn").click(function(){
		...
		let param = "/Proj_OnedayClass/qnaBBS/QnaReply2.jsp?";
			param += "qNum="+qNum;
			param += "&nowPage="+nowPage;
			param += "&keyField="+p3;
	     	param += "&keyWord="+p4; 
	...
	$("td.reply>button#replyBtn").click(function(event){
		...
		if (qTitle == "") {
		    ...
		    $.ajax({             
		    	type: "POST",
		        url: "/Proj_OnedayClass/qnaBBS/QnaReplyProc2.jsp",        
		        data: formData,            
		        success: function () {
					...
					let param = "/Proj_OnedayClass/qnaBBS/QnaList2.jsp?nowPage=" + nowPage;		    
	    			param += "&keyField="+keyField;
     				param += "&keyWord="+keyWord;
	...
	$("td.update>button#modBtn").click(function(event){
		...
		if (qTitle == "") {
		    ...
		    $.ajax({             
		    	type: "POST",
		        url: "/Proj_OnedayClass/qnaBBS/QnaUpdateProc2.jsp",        
		        data: formData,            
		        success: function () {
					...
					let param = "/Proj_OnedayClass/qnaBBS/QnaRead2.jsp?nowPage=" + nowPage;
					param += "&qNum="+qNum;
	    			param += "&keyField="+keyField;
     				param += "&keyWord="+keyWord;
